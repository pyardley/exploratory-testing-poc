...
All 9 recommendations below are now implemented (previously this section was "not yet implemented"). Grounded in the BrightBasket validation (`FINDINGS.md` §14 and `external-validation/brightbasket/README.md`): fixing the checkout-walkthrough gap took static-pipeline recall from 25% to ~45%, but a category-level look at what was *still* missed on that same run pointed at several more scope gaps with the same shape as Recommendations #1–#2 above — narrow, fixable, not a fundamental judgement problem — plus one genuine regression the checkout-walkthrough fix itself introduced.

Unlike the first "Recommendations for Improvement" batch above, this batch has **not** been re-validated with a fresh measured pipeline run against the answer key — the Results table above still reflects only the first batch, and no new before/after numbers exist for these nine. What did happen: a `--skip-ai` deterministic-only smoke run against WidgetWorks (no API cost) confirmed all nine changes run cleanly end-to-end and no-op harmlessly on pages that don't match their pattern (WidgetWorks has no checkout/discount/stepper/payment surface at all, so items #1, #3–#7 no-op there entirely by design). That same smoke run also immediately produced two new deterministic findings landing squarely on faults `FINDINGS.md` §11 named as blind spots *neither* pipeline had ever caught: an `image-aspect-ratio-distorted` flag on `w-009.svg`/`w-021.svg`/etc. (the `I-01`/`I-02` shape) and a `card-hover-state-missing` deviation on `catalog.html` vs. the golden page's own hovering cards (the `F-03` shape) — encouraging, but not a substitute for a full measured recall number.

1. ✅ **A discount/promo-code interaction prober.** Implemented as `probeDiscountCode()` in `exploratory-tester/src/scanners/interactionProbe.js`: locates a text input near a submit-like control whose label/placeholder matches `/discount|coupon|promo|voucher/i`, submits an obviously-invalid code, and confirms both that an error is shown *and* that no `/total/i`-labeled figure on the page actually changed (catching "error shown but applied anyway" generically). Where the app description documents real codes — extracted by `extractDocumentedCodes()` in `scan.js` via a generic backtick/quote-token pattern near discount-context wording, not any one app's exact phrasing — it also tries each documented code plus a case-flipped variant and reports whether acceptance differs by case alone. No-ops (`discountCodeProbe: null`) on any page with no discount-code field at all, confirmed on WidgetWorks.
2. ✅ **A quantity-stepper boundary prober.** Implemented as `probeQuantityStepper()` in `interactionProbe.js`: generically tags matching increase/decrease button pairs (ARIA `increase`/`decrease`/`increment`/`decrement` labels, falling back to plain `+`/`-` text buttons), clicks the "+" side 15 times and the "−" side 30 times — well past any plausible limit — and reports whether the displayed value ever wrapped back down instead of capping, or went below zero.
3. ✅ **A payment-field boundary prober, cross-checked against the page's own hint text.** Implemented as `probePaymentFieldBoundaries()` in `exploratory-tester/src/scanners/formValidation.js`: tries boundary-length test numbers (valid length, one digit short, one digit long) for Visa/Mastercard/Amex against any card-number-shaped field, captures the page's own nearby hint text, and flags `lengthAcceptanceContradictions` — a valid-length number that still errors, or a wrong-length one that doesn't. Uses the page's own copy as the oracle (always available) rather than parsing the business-rules doc, which the recommendation named as an *alternative* oracle, not the only one.
4. ✅ **Sample more than one record/session where the fault catalog shows sampling matters.** Two independent changes: `dedupePagesByPathname()` in `scan.js` now keeps a *second* distinct record for any detail/item-shaped URL (a query string or numeric path segment) rather than always taking the first one the crawler found — confirmed on the WidgetWorks smoke run, which sampled both `item.html?id=w-009` and `item.html?id=w-021`. Separately, `checkoutWalkthrough.js` now places a cheap second order after the first (`placeQuickSecondOrder()`) and diffs any `findOrderCountStat()`-matched "N orders" text between the two visits, to catch a stat cached on first load.
5. ✅ **Close the regression this fix itself introduced: also test the empty-basket path once.** Implemented as `checkEmptyBasketCheckoutPath()` in `checkoutWalkthrough.js`, run as the very first step (before anything else ever adds an item): clears the basket via any `remove`/`delete`-labeled control, then attempts checkout anyway and records whether a checkout control is even visible and, if so, whether it actually advances — recovering the original `U-01`-shaped catch without giving up the non-empty-basket coverage gained since.
6. ✅ **Tag scaffolding actions in the evidence bundle so the AI review can't mistake them for organic behavior.** `runCheckoutWalkthrough()` now returns a structured `scaffoldingActions` array (populated for both the "leave one item in the basket" step and the second-order stat check), threaded through `scan.js` into every page's `manifest.json` as `scaffoldingActionsThisSession` and surfaced as its own labeled section in the per-page AI review prompt (`exploratory-tester/src/ai/prompts/perPageReview.md`), with an explicit instruction not to cite anything traceable to one as organic behavior.
7. ✅ **Make multi-run adjudication resilient to the platform-specific concurrent-spawn flakiness found while scoring this.** `runMultipleAdjudications()` in `comparison/src/adjudicate.js` now runs sequentially with retry-with-backoff (up to 2 retries) on a bare `exited 1` (empty stderr) specifically on `process.platform === "win32"`, as a platform-detected fallback — other platforms keep the original concurrent `Promise.all` path, since the flakiness was Windows/Git-Bash-specific.
8. ✅ **Hover/focus pseudo-state diffing.** Implemented as `extractCardHoverFacts()` in `exploratory-tester/src/scanners/visualConsistency.js`, folded into `extractVisualFacts()`'s return value: a real Playwright `hover()` (not simulatable via `page.evaluate()`) on the page's first card-like element, diffing box-shadow/transform/background/border/opacity before vs. after, then compared in `compareToGolden()` against the golden page's own hover delta. Caught a real `card-hover-state-missing` deviation on WidgetWorks' `catalog.html` in the smoke run above — the exact `F-03` shape `FINDINGS.md` §11 named as previously uncaught.
9. ✅ **Image aspect-ratio computation.** Implemented as `checkImageAspectRatios()` in `visualConsistency.js`, folded into `extractVisualFacts()` and turned into deviations by `imageAspectRatioDeviations()` regardless of golden-facts availability (a self-referential per-image check needs no golden comparison at all — an `<img>`'s own natural aspect ratio is the oracle). Caught real `image-aspect-ratio-distorted` deviations on WidgetWorks' product thumbnails (`w-009.svg`, `w-021.svg`, and others, all ~21% off) in the smoke run above — the exact `I-01`/`I-02` shape `FINDINGS.md` §11 named as an LLM-vision blind spot in both prior pipelines.
