...
Not yet implemented. Grounded in the BrightBasket validation (`FINDINGS.md` §14 and `external-validation/brightbasket/README.md`): fixing the checkout-walkthrough gap took static-pipeline recall from 25% to ~45%, but a category-level look at what's *still* missed on that same run points at several more scope gaps with the same shape as Recommendations #1–#2 above — narrow, fixable, not a fundamental judgement problem — plus one genuine regression the checkout-walkthrough fix itself introduced.

1. **A discount/promo-code interaction prober.** BrightBasket's entire discount-code surface (`U-07` — an expired code shows an error yet still gets applied; `U-08` — code entry is silently case-sensitive; `CL-05` — a documented-valid code is rejected as expired; `W-08` — the applied discount doesn't match its own stated percentage) went unscored because nothing ever tries a discount code at all. A generic scanner — sibling to `formValidation.js` and `interactionProbe.js`, not BrightBasket-specific — could: locate a text input near a submit-like control whose label/placeholder matches `/discount|coupon|promo|voucher/i`; submit an obviously-invalid value and confirm both that an error is shown *and* that no total on the page actually changed (catching the "error shown but applied anyway" shape generically, not just this app's specific bug); and, where the app-description input documents real codes (as BrightBasket's business rules do), try each one plus a case-flipped variant, diffing the displayed discount amount against `stated percent × subtotal` the same way `formValidation.js` already diffs error *text*, not just a pass/fail boolean.
2. **A quantity-stepper boundary prober.** `W-07` (a `+`/`−` stepper silently wraps back to 1 instead of capping past 10) and `CRUD-07` (the `−` side has no floor and goes negative) are exactly the kind of "perform the action past its sane range and check the outcome" bug the existing dropdown/pagination probes in `interactionProbe.js` already catch for other control types. Extending that same file with a stepper-shaped probe — find a numeric input flanked by two buttons (or matching ARIA `Increase/Decrease quantity` labels), click one side repeatedly well past any plausible limit, assert the displayed value is monotonic and never wraps or goes negative — is a natural sibling addition, not a new architecture.
3. **A payment-field boundary prober, cross-checked against the app's own stated business rules.** `W-01`/`W-02` (wrong-length card numbers silently accepted for Visa/Mastercard, correct-length Amex numbers wrongly rejected) and `CL-03` (the on-page hint text disagrees with the actual accepted length for one card brand) all need someone to actually *try* boundary-length card numbers and compare the result against what the app claims — either in its own hint text or in the product description's documented business rules. This generalizes `formValidation.js`'s existing "two scenarios, diff the exact behavior" approach from "empty vs. wrong-type" to "boundary-valid vs. one-off-each-side," with the business-rules doc (already a required CLI input) as an additional oracle beyond the page's own copy.
4. **Sample more than one record/session where the fault catalog shows sampling matters.** `CRUD-03` (one specific product's detail page always 500s) was missed for the same reason WidgetWorks' analogous `CRUD-03` was in the very first baseline (see Conclusion #4 above) — only one item got visited. `H-03` (an "orders placed" stat that's cached on first load and never updates) needs a *second* order in the same session to even be observable. The `checkoutWalkthrough.js` scanner added for BrightBasket is a natural place to extend this: place a second order after the first (cheap — it's already driving the flow once) and diff any count/stat-shaped text between the two visits, and have the product-detail evidence pass sample more than one item id rather than just whichever the crawler happened to discover first.
5. **Close the regression this fix itself introduced: also test the empty-basket path once.** `U-01` ("basket can be empty and checkout still goes through") was **caught before this fix and missed after it** — a direct, honest side effect of `checkoutWalkthrough.js` deliberately leaving the basket non-empty afterward so the *rest* of that page's surface stops being invisible (see `FINDINGS.md` §14). Fix: have the walkthrough attempt the empty-basket-checkout path too, as a distinct, cheap sub-check (clear the basket, try to reach payment, note whether anything blocks it) before doing its normal non-empty run — recovering the original catch without giving up the coverage gained since.
6. **Tag scaffolding actions in the evidence bundle so the AI review can't mistake them for organic behavior.** The same walkthrough's "leave one item in the basket for the next evidence pass" step produced a real false positive: AI review credited it as proof "the basket isn't cleared after checkout," when it's actually a *second, unrelated* item the scanner itself added moments later. Any scanner action taken purely to shape evidence for a *later* capture (not because the checklist called for it) should be recorded in the manifest (e.g. `scaffoldingActions: ["added 1 unrelated item to basket after order placement, for basket.html's own evidence capture"]`) and the AI review prompt should be told to discount observations that trace back to one.
7. **Make multi-run adjudication resilient to the platform-specific concurrent-spawn flakiness found while scoring this.** `comparison/compare.js --runs N` fires `N` `claude -p` calls via `Promise.all`; on this project's Windows/Git-Bash setup those parallel spawns failed outright (`exited 1`, empty stderr) noticeably more often than the exact same call run alone, sequentially — see `FINDINGS.md` §14. This isn't a detection-recall issue, but it blocks *trusting* any recall number computed with `--runs N > 1` until it's fixed: run adjudications sequentially with a short retry-with-backoff on a bare `exited 1` instead of firing them all via `Promise.all`, at least as a platform-detected fallback.
8. **Hover/focus pseudo-state diffing.** `FINDINGS.md` §11 names this as one of only 4 faults *neither* the static nor the agentic pipeline ever caught on WidgetWorks: `F-03` (product cards show no `:hover` feedback, unlike the Home page's own feature cards) is invisible because nothing in this tool ever captures or diffs a hover/focus state at all — every screenshot and DOM snapshot is taken at rest. Fix: for each interactive/card-like element already being captured, script a hover (`page.hover(selector)`), take a follow-up screenshot/computed-style read, and diff it against the golden reference's own hover behavior for an equivalent element — the same "does this element's own state match what an equivalent element elsewhere does" comparison `visualConsistency.js` already runs for resting-state properties (color/radius/spacing), just triggered once more under `:hover`.
9. **Image aspect-ratio computation.** Also named in `FINDINGS.md` §11: WidgetWorks' `I-01`/`I-02` (a squashed logo, squashed product thumbnails) sat in the same screenshot evidence the AI review already had — *twice*, once in the static pipeline and again independently in the agentic run — and neither ever flagged it, a genuine LLM-vision blind spot for this specific defect shape (subtle proportion distortion), not a missing scanner. Deliberately *not* an AI-judgement fix: compute each `<img>`'s natural (`naturalWidth`/`naturalHeight`) vs. rendered (`clientWidth`/`clientHeight`) aspect ratio in `visualConsistency.js`'s existing golden-comparison pass, and flag any mismatch past a small tolerance — exactly the mechanism already used for button radius and spacing, applied to one more computed fact.
